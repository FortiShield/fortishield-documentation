...
.. meta::
  :description: Learn how to monitor your organization's activity via Fortishield's integration with the Microsoft Graph API in this section of our documentation.
...
In turn, this module integrates with Microsoft Graph to bring this content-rich source of information into Fortishield. This is done through a series of `resources` and `relationships`, which describe the function, type, and content of various logs available within Microsoft Graph.
...
Before **Fortishield** can begin pulling logs and other content from the Microsoft Graph API, it must be authorized and pass through an authentication process. To authenticate, Fortishield must provide the ``tenant_id``, ``client_id``, and ``secret_value`` of an authorized application, which we will register through Azure.
...
#. Fill in the name of your app, choose the desired account type, and click on the **Register** button:
   ...
   .. thumbnail:: /images/cloud-security/ms-graph/1-azure-fortishield-app-register-application.png
       :title: Register your app
       :alt: Register your app
       :align: center
       :width: 100%
...
#. The app is now registered, and you can see information about it in its **Overview** section. Make sure to note down the ``client_id`` and ``tenant_id`` information:
   ...
   .. thumbnail:: /images/cloud-security/ms-graph/2-azure-fortishield-app-overview.png
       :title: Register your app
       :alt: Register your app
       :align: center
       :width: 100%
...
#. Generate a secret to be used during the authentication process. Go to **Certificates & secrets** and click on **New client secret**, which will then generate the secret and its ID:
   ...
   .. thumbnail:: /images/cloud-security/ms-graph/3-azure-fortishield-app-create-secret.png
...
#. Ensure that the ``secret_value`` information is copied down and saved:
    ...
    .. thumbnail:: /images/cloud-security/ms-graph/3-azure-fortishield-app-create-secret-copy-value.png
...
#. Add the following relationships' permissions under the **SecurityAlert** and **SecurityIncident** sections:
   ...
   .. thumbnail:: /images/cloud-security/ms-graph/4-azure-fortishield-app-configure-permissions.png
...
.. thumbnail:: /images/cloud-security/ms-graph/4-azure-fortishield-app-configure-permissions-admin-consent.png
...
Configure the ``ms-graph`` module in the Fortishield manager or in the Fortishield agent :doc:`configuration file </user-manual/reference/ossec-conf/index>`. Through the following configuration, Fortishield is ready to search for logs created by Microsoft Graph resources and relationships.
...
One of the more ubiquitous alerts that an organization of any size receive is spam emails. In this case, we can specifically look at an example where the spam email contains malicious content, and examine how Microsoft Graph & Fortishield report on this information.
...
The Fortishield manager includes a set of pre-made rules that aid in classifying the importance and context of different events. 
...
Once Fortishield connects with the Microsoft Graph API, the previous log triggers the rule and raises the following alert:
