...
.. meta::
  :description: Azure Storage refers to the Microsoft Azure cloud storage solution. Learn how to use Azure Storage with Fortishield in this section of our documentation.
...
As an alternative to the Azure Log Analytics REST API, Fortishield offers the possibility to access Azure Storage accounts in a simple way. The activity logs of the Microsoft Azure infrastructure can be exported to the storage accounts.
...
Thanks to the following rules, already included in the default Fortishield ruleset, it is possible to monitor the infrastructure activity and obtain related alerts:
...
Once the Fortishield configuration is set and the ``azure-logs`` module is running using the previous configuration, the event from the user creation example exported to Storage will be processed. The results can be checked in the Fortishield dashboard. 
