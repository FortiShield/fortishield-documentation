...
This case uses a public IP reputation database that contains the IP addresses of some malicious actors. An IP reputation database is a collection of IP addresses that have been flagged as malicious. The RHEL endpoint plays the role of the malicious actor here, therefore you add its IP address to the reputation database. Then, configure Fortishield to block the RHEL endpoint from accessing web resources on the Apache web servers for 60 seconds. It’s a way of discouraging attackers from continuing to carry out their malicious activities.

In this use case, you use the Fortishield :doc:`CDB list </user-manual/ruleset/cdb-list>` and :doc:`active response </getting-started/use-cases/incident-response>` capabilities.
...
+===============+=====================================================================================================================================================================+
| RHEL 9.0      | Attacker endpoint connecting to the victim's web server on which you use Fortishield CDB list capability to flag its IP address as malicious.                             |
+---------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Ubuntu 22.04  | Victim endpoint running an Apache 2.4.54 web server. Here, you use the Fortishield active response module to automatically block connections from the attacker endpoint.  |
+---------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Windows 11    | Victim endpoint running an Apache 2.4.54 web server. Here, you use the Fortishield active response module to automatically block connections from the attacker endpoint.  |
+---------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Perform the following steps to install an Apache web server and monitor its logs with the Fortishield agent.
...
#. Add the following to ``/var/ossec/etc/ossec.conf`` file to configure the Fortishield agent and monitor the Apache access logs:
...
Configure the Fortishield agent
~~~~~~~~~~~~~~~~~~~~~~~~~

Perform the steps below to configure the Fortishield agent to monitor Apache web server logs.

#. Add the following to ``C:\Program Files (x86)\ossec-agent\ossec.conf`` to configure the Fortishield agent and monitor the Apache access logs:
...
#. Restart the Fortishield agent in a PowerShell terminal with administrator privileges to apply the changes:
...
You need to perform the following steps on the Fortishield server to add the IP address of the RHEL endpoint to a CDB list, and then configure rules and active response.
...
#. Download a script to convert from the ``.ipset`` format to the ``.cdb`` list format:
   ...
   .. code-block:: console
      ...
      $ sudo wget https://fortishield.github.io/resources/iplist-to-cdblist.py -O /tmp/iplist-to-cdblist.py
...
#. Assign the right permissions and ownership to the generated file:
   ...
   .. code-block:: console
      ...
      $ sudo chown fortishield:fortishield /var/ossec/etc/lists/blacklist-alienvault

Configure the active response module to block the malicious IP address
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

#. Add a custom rule to trigger a Fortishield :doc:`active response </user-manual/capabilities/active-response/index>` script. Do this in the Fortishield server ``/var/ossec/etc/rules/local_rules.xml`` custom ruleset file:
...
#. Edit the Fortishield server ``/var/ossec/etc/ossec.conf`` configuration file and add the ``etc/lists/blacklist-alienvault`` list to the ``<ruleset>`` section:
...
#. Add the active response block to the Fortishield server ``/var/ossec/etc/ossec.conf`` file:
...
The attacker endpoint connects to the victim's web servers the first time. After the first connection, the Fortishield active response module temporarily blocks any successive connection to the web servers for 60 seconds.
