...
.. meta::
   :description: Learn how to install the Ansible server in this section of the Fortishield documentation. Check out this step-by-step guide.
...
Our Ansible server will need to connect to the other endpoints. Let’s see how to make this connection between our ansible server and the machine where we will install the Fortishield server. This procedure has to be repeated for each machine we want to connect to the Ansible server. For example, the endpoints where Fortishield agents will be deployed.
...
#. Now, proceed to copy the public key of the Ansible server to the  ~/.ssh/authorized_keys file in the $HOME directory of the remote system (the Fortishield server in this example).
...
#. Return to the Ansible server and add the public key (``id_rsa.pub``) of the Ansible server to the ``~/.ssh/authorized_keys`` file in the $HOME directory of the Fortishield server using SSH.
   ...
   #. When we read the Fortishield server ``~/.ssh/authorized_keys``, we can see it contains the public key of the ansible server.

      .. code-block:: console

         $ cat .ssh/authorized_keys

#. Before the public key authentication mechanism can be tested, we have to verify that the SSH configuration on the remote endpoint allows it. To do this, open the file ``/etc/ssh/sshd_config`` on the Fortishield server.
...
#. Add endpoints for management by Ansible.
   ...
   This is done by including the hostname or IP Address in ``/etc/ansible/hosts`` on our Ansible server. In this case, we intend to use the Ansible playbooks to deploy the Fortishield indexer, dashboard, and manager on one server (all-in-one deployment). The IP address of the server is ``192.168.33.31`` and the user is ``centos``.
...
We can obtain the necessary playbooks and roles for the installation of the Fortishield server components,  and Fortishield agents by cloning the fortishield-ansible repository in ``/etc/ansible/roles``.
...
.. code-block:: console
   ...
   # sudo git clone --branch v|FORTISHIELD_CURRENT_ANSIBLE| https://github.com/fortishield/fortishield-ansible.git
   # ls
