...
.. meta::
   :description: Find out how to integrate Fortishield with Elastic in this integration guide.
...
In this guide, you can find out how to integrate Fortishield with Elastic in the following ways:
...
#. Run the following command to install the `logstash-input-opensearch <https://github.com/opensearch-project/logstash-input-opensearch>`__ plugin. This plugin reads data from the Fortishield indexer into the Logstash pipeline.

   .. code-block:: console

      $ sudo /usr/share/logstash/bin/logstash-plugin install logstash-input-opensearch

#. Copy the Fortishield indexer and Elasticsearch root certificates to the Logstash server. 

   .. note::

      You can add the certificates to any directory of your choice. For example, we added them in ``/etc/logstash/fortishield-indexer-certs`` and ``/etc/logstash/elasticsearch-certs`` respectively.
...
#. Give the ``logstash`` user the necessary permissions to read the copied certificates:
   ...
   Replace ``</PATH/TO/LOCAL/FORTISHIELD-INDEXER/CERTIFICATE>/root-ca.pem`` and ``</PATH/TO/LOCAL/ELASTICSEARCH/CERTIFICATE>/root-ca.pem`` with your Fortishield indexer and Elasticsearch certificate local paths on the Logstash endpoint respectively.
...
A `Logstash pipeline <https://www.elastic.co/guide/en/logstash/current/configuration.html>`__ allows Logstash to use plugins to read the data from the Fortishield indexes and send them to Elasticsearch.
...
#. Run the following commands to securely store the credentials of the Fortishield indexer and Elasticsearch in the Logstash keystore.
   ...
   .. note::
      ...
      ``ELASTICSEARCH_USERNAME``, ``ELASTICSEARCH_PASSWORD``, ``FORTISHIELD_INDEXER_USERNAME``, and ``FORTISHIELD_INDEXER_PASSWORD`` are `keys <https://www.elastic.co/guide/en/logstash/current/keystore.html>`__ representing the secret values you are adding to the Logstash keystore. These keys will be used in the Logstash pipeline.
...
#. Perform the following steps to configure the Logstash pipeline.
   ...
   #. Create the configuration file ``fortishield-elasticsearch.conf`` in ``/etc/logstash/conf.d/`` folder:

      .. code-block:: console

         $ sudo touch /etc/logstash/conf.d/fortishield-elasticsearch.conf
   
   #. Add the following configuration to the ``fortishield-elasticsearch.conf`` file. This sets the parameters required to run Logstash.
      ...
      .. code-block:: none
         ...
         input {
           opensearch {
            ...
            ssl => true
            ca_file => "</PATH/TO/LOCAL/FORTISHIELD_INDEXER>/root-ca.pem"
            query =>  '{
                "query": {
                   "range": {
                      "@timestamp": {
      ...
      -  ``</PATH/TO/LOCAL/FORTISHIELD_INDEXER>/root-ca.pem`` is your Fortishield indexer certificate local path on the Logstash server. For example,  you can use ``/etc/logstash/fortishield-indexer-certs/root-ca.pem`` which is the Fortishield indexer root certificate that was copied earlier.
...
#. Once you have everything set, run Logstash from CLI with your configuration:
   ...
   .. code-block:: console
      ...
      $ sudo -E /usr/share/logstash/bin/logstash -f /etc/logstash/conf.d/fortishield-elasticsearch.conf --path.settings /etc/logstash/
...
After Logstash is successfully running, check how to :ref:`configure the Fortishield alert index pattern <configuring_fortishield_alerts_index_pattern_in_elastic>` and :ref:`verify the integration <verifying_elastic_integration>`.
...
We use Logstash to forward security data in the ``/var/ossec/logs/alerts/alerts.json`` alerts file from the Fortishield server to the Elasticsearch indexes.
...
#. Copy the Elasticsearch root certificate to the Fortishield server. You can add the certificate to any directory of your choice. In our case, we add it in ``/etc/logstash/elasticsearch-certs`` directory.
...
#. Give the ``logstash`` user the necessary permissions to read the copied certificates:
   ...
   Replace ``</PATH/TO/LOCAL/ELASTICSEARCH/CERTIFICATE>/root-ca.pem`` with your Elasticsearch certificate local path on the Fortishield server.
...
A `Logstash pipeline <https://www.elastic.co/guide/en/logstash/current/configuration.html>`__ allows Logstash to use plugins to read the data in the Fortishield ``/var/ossec/logs/alerts/alerts.json`` alert file and send them to Elasticsearch.
...
#. Perform the following steps to configure the Logstash pipeline.
   ...
   #. Add the following configuration to the ``fortishield-elasticsearch.conf`` file. This sets the parameters required to run Logstash.
      ...
      -  ``<ELASTICSEARCH_ADDRESS>`` is your Elasticsearch IP address.
      -  ``</PATH/TO/LOCAL/ELASTICSEARCH>/root-ca.pem`` is your Elasticsearch root certificate local path on the Fortishield server. For example, you can use ``/etc/logstash/elasticsearch-certs/root-ca.pem`` which is the Elasticsearch root certificate that was copied earlier.
...
.. _configuring_fortishield_alerts_index_pattern_in_elastic:

Configuring the Fortishield alerts index pattern in Elastic
-----------------------------------------------------

In Kibana, do the following to create the index pattern name for the Fortishield alerts.

#. Select **☰** > **Management** > **Stack Management**.
#. Choose **Kibana** > **Data Views** and select **Create data view**. 
#. Enter a name for the data view and define ``fortishield-alerts-*`` as the index pattern name.
...
.. thumbnail:: /images/integrations/configuring-index-pattern-in-elastic.gif
   :title: Configuring the Fortishield alerts index pattern in Elastic
   :align: center
   :width: 80%
...
To check the integration with Elasticsearch, navigate to **Discover** in Kibana and verify that you can find  the Fortishield security data with the data view name you entered.
...
Importing these dashboards defines the index pattern name ``fortishield-alerts-*``. The index pattern name is necessary for creating index names and receiving the alerts. We recommend using ``fortishield-alerts-4.x-%{+YYYY.MM.dd}``.

Follow the next steps to import the Fortishield dashboards for Elastic.

#. Run the command below to download the Fortishield dashboard file for Elastic.
