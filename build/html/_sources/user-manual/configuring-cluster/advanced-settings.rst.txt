...
.. meta::
  :description: Learn more about how to deploy a Fortishield cluster. In this section of our documentation we explain more about the agents connections. 
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    In Fortishield's case, users want to use a load balancer to catch all the agent events and distribute them between the different workers in the cluster.
    The correct way to use it is to point every agent to send the events to the *load balancer*:

    1. Edit the Fortishield agent configuration in ``/var/ossec/etc/ossec.conf`` to add the **Load Balancer** IP address. In the ``<client><server>`` section, change the ``LOAD_BALANCER_IP`` value to the ``load balancer`` address and ``port``:
            ...
            .. code-block:: nginx
                ...
                stream {
                    upstream cluster {
                        ...
                        server <FORTISHIELD-MASTER-IP>:1514;
                        server <FORTISHIELD-WORKER1-IP>:1514;
                        server <FORTISHIELD-WORKER2-IP>:1514;
                    }
                    upstream master {
                        server <FORTISHIELD-MASTER-IP>:1515;
                    }
                    server {
                        listen 1514;
                        proxy_pass cluster;
