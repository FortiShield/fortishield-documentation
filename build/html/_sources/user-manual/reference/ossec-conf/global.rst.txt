...
.. meta::
  :description: The ossec.conf file is the main configuration file on the Fortishield manager and plays an important role on the agents. Learn more about the global configuration here.
...
This option defines how the Fortishield server identifies itself when sending mail.
...
.. warning::
   ...
   Disabling ``jsonout_output`` disrupts the alerts indexing process. In a default installation, Filebeat reads the ``alerts.json`` file and sends this information to the Fortishield indexer. If the writing to this file stops, alerts are no longer indexed.
...
This setting toggles whether to query the external Fortishield Cyber Threat Intelligence (CTI) service for any available Fortishield updates.
