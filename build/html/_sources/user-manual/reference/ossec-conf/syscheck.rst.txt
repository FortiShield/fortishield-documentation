...
.. meta::
  :description: The ossec.conf file is the main configuration file on the Fortishield manager and also important on the agents. Learn more about FIM settings with configuration examples here.
...
This option can be used to limit the size of the ``queue/diff/local`` folder where Fortishield stores the compressed files used to perform the diff operation when ``report_changes`` is enabled. After reaching this size, alerts will not show the diff information until the size is smaller than the configured limit.
...
.. tabs::
 ...
 .. group-tab:: Fortishield agent - Linux/Unix
 ...
 .. group-tab:: Fortishield agent - Windows
 ...
 .. group-tab:: Fortishield agent - MacOS X
