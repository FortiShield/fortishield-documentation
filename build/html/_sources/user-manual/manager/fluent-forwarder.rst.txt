...
.. meta::
  :description: The Fluentd forwarder module allows Fortishield to forward messages to a Fluentd server. Learn more about it in this section of our documentation.
...
This module allows Fortishield to forward messages to a Fluentd server. Fluentd it's an open source data collector logger that comes along
...
.. thumbnail:: ../../images/fluent-forward/fortishield-fluentd.png
  :title: Fortishield fluent forwarder flow diagram
  :align: center
  :width: 100%
...
- **Forward alerts generated by Fortishield using analysisd**. In this configuration, you set the socket to which forward the alerts.

Example using logcollector
^^^^^^^^^^^^^^^^^^^^^^^^^^

This example is for testing purposes on a Debian machine, with the Fortishield manager installed.
