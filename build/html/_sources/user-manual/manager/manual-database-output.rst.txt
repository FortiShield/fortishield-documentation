...
.. meta::
  :description: In order to configure Fortishield so it outputs alerts to a database, users must compile Fortishield with the database type desired. Learn more about this process here.
...
It's possible to configure Fortishield to output the alerts into a database. To do this, users must compile Fortishield with the database type that users want to use.
...
As previously mentioned, the database output can be enabled when compiling Fortishield with the database type to use. On the :doc:`sources installation guide </deployment-options/fortishield-from-sources/fortishield-server/index>`, users must pre-compile the source code before running the ``install.sh`` script.
...
The compilation process might take some time. After finishing this process, please continue with the sources' installation guide. Now Fortishield will be installed with database support, but we must enable manually the feature after configuring it.

Database configuration
----------------------

Now that we have Fortishield installed with database support, we need to set up the database server. We'll create a new database, set up the database user, and add the schema (located in the ``src/os_dbd`` directory of the source code) with the following commands, according to your database system:
...
In order for Fortishield to output alerts and other data into the database, the users must add a ``<database_output>`` section on the configuration file, located at ``/var/ossec/etc/ossec.conf``. Fill in the block with the right database name and credentials. The hostname must be the IP address of the database server.
...
The setup process for the database output is finished. Now the only thing left is to restart the Fortishield manager:
