...
.. meta::
   :description: Google Workspace is a collection of cloud computing, productivity and collaboration tools. Learn more about it and the administrator role in this section of the Fortishield documentation.
...
#. Define the attribute for users.
   ...
   #. Add a value to the **Department** field, in this example, we add ``Fortishield_access``, click on **SAVE**. This value will be used in the ``role_mapping`` file configuration.
...
#. Edit the ``/etc/fortishield-indexer/opensearch-security/roles_mapping.yml`` file and change the following values:
   ...
   Map the ``Department`` field value that was obtained in Google IdP to the ``all_access`` role in the Fortishield indexer:
   ...
   .. code-block:: console
      ...
      all_access:
        ...
        - "admin"
        - "Fortishield_access"
        description: "Maps admin and Fortishield_access to all_access"
...
#. Check the value of ``run_as`` in the ``/usr/share/fortishield-dashboard/data/fortishield/config/fortishield.yml`` configuration file. If ``run_as`` is set to ``false``, proceed to the next step.
   ...
   #. Click **Create Role mapping** and complete the empty fields with the following parameters:
      ...
      - **Value**: Assign the Department field value that was obtained in Google IdP, in our case, this is ``Fortishield_access``.   

      .. thumbnail:: /images/single-sign-on/google/Fortishield-role-mapping.png
