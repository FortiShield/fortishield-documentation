...
.. meta::
  :description: Learn more about the agent upgrade module of Fortishield, responsible for carrying out the entire agent upgrade process remotely. 
...
.. code-block:: none
    ...
    {FORTISHIELD_DIR}/queue/tasks/upgrade
...
2. **Upgrade custom parameters**: Command to upgrade agents from a local WPK file.
    ...
    .. note:: In case of having a multi-node Fortishield cluster, the custom WPK file has to exist on all nodes in the specified path.
...
.. code-block:: none
    ...
    {FORTISHIELD_DIR}/queue/tasks/task

.. note:: In case of having a multi-node Fortishield cluster, the task manager only runs on the master node. This request should always go to the master node.
