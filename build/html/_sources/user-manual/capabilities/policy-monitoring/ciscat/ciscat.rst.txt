...
.. meta::
  :description: The CIS-CAT wodle has been developed in order to integrate CIS benchmark assessments into Fortishield agents. Learn more about it. 
...
The **CIS-CAT wodle** has been developed for the purpose of integrating CIS benchmark assessments into Fortishield agents.
...
The CIS-CAT Fortishield module integrates CIS benchmark assessments into Fortishield agents and reports the results of each scan in the form of an alert.
...
1. In the configuration file, ``ossec.conf``, set up a section as follows:
    ...
    Make sure the paths are correct for the location of your Java and the CIS-CAT tool. For both cases, you could specify the full path, or a relative path to the Fortishield installation folder. Also, consider the following tips when configuring the ``content`` section:
...
2. After restarting the Fortishield agent, the benchmark checks will be executed at the specified interval, triggering alerts as shown below.
...
Since Fortishield v3.5.0, the report summary is stored in the agent DB to query it by the Fortishield API. This allows knowing about the last scan every time the user wants to.
