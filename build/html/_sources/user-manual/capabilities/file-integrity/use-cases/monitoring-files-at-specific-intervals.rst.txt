...
2. Edit the Fortishield agent ``/var/ossec/etc/ossec.conf`` configuration file and add the ``user_details.txt`` file for monitoring:
...
3. Restart the Fortishield agent to apply the configuration:
...
Navigate to **File Integrity Monitoring** on the Fortishield dashboard to view the alert generated when the FIM module detects changes to the ``user_details.txt`` file.
