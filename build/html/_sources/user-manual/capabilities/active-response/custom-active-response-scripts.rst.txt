...
Fortishield allows you to program stateless custom active responses in any programming language. They need to be able to perform the following actions for proper execution:
...
#. Change the active response script ownership and permissions as shown below:
   ...
   .. code-block:: console
      ...
      $ sudo chown root:fortishield /var/ossec/active-response/bin/custom-ar.py
...
#. Add the ``<command>`` and ``<active-response>`` blocks below to the Fortishield server ``/var/ossec/etc/ossec.conf`` file to use this active response Python script:
...
#. Change the active response script permissions and ownership as shown below:
   ...
   .. code-block:: console
      ...
      $ sudo chown root:fortishield /Library/Ossec/active-response/bin/custom-ar.py

#. Restart the Fortishield agent to apply the changes:

   .. code-block:: console

      $ sudo /Library/Ossec/bin/fortishield-control restart

#. Add the ``<command>`` and ``<active-response>`` blocks below to the Fortishield server ``/Library/Ossec/etc/ossec.conf`` file to use this active response Python script:
...
#. On the Fortishield server, add the ``<command>`` and ``<active-response>`` blocks below to the ``/var/ossec/etc/ossec.conf`` configuration file. This uses  the ``custom-ar.exe`` executable for Windows endpoints.
...
With this configuration, Fortishield runs an executable instead of a Python script when triggering an active response on a Windows endpoint.

Method 2: Run a Python script through a Batch launcher
""""""""""""""""""""""""""""""""""""""""""""""""""""""

In this method, the Fortishield active response module executes the ``launcher.cmd`` script which subsequently executes the ``custom-ar.py`` script.
...
#. On the Fortishield server, add the ``<command>`` and ``<active-response>`` blocks below to the Fortishield configuration ``/var/ossec/etc/ossec.conf`` file. The active response module runs the ``launcher.cmd`` script which runs the Python script in its ``<extra_args>`` option. This action executes for 60 seconds when rule ID 503 is triggered.
