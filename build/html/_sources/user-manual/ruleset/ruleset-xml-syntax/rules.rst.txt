...
.. meta::
  :description: The Fortishield Ruleset is used to analyze incoming events and generate alerts when appropriate. Learn more about Rules syntax in this section.

Rules Syntax
============

The Fortishield Ruleset combined with any customs rules is used to analyze incoming events and generate alerts when appropriate. The Ruleset is in constant expansion and enhancement thanks to the collaborative effort of our developers and our growing community.

Our aim is to provide the best guidance possible for anyone who may be looking into developing their own custom rules. Remember that you can always :doc:`contribute <../contribute>` to our community.
...
+-------------------------+---------------------------------------------------------------+------------------------------------------------------------------------------------------------------+
| :ref:`rules_category`   | Any :ref:`type <decoders_type>`.                              | It will match with logs whose decoder's :ref:`type <decoders_type>` concur.                          |
+-------------------------+---------------------------------------------------------------+------------------------------------------------------------------------------------------------------+
...
Groups categorize alerts. They allow filtering related alerts in the Fortishield dashboard.

The default Fortishield ruleset already includes rules that use groups like ``syscheck,``, ``attack,``, and ``syslog,``. As an example, you can filter alerts for these categories by querying ``rule.groups: attack`` or ``rule.groups: (syscheck OR syslog)`` in the Fortishield dashboard.
...
Example:
  ...
  .. code-block:: xml
    ...
    <rule id="140101" level="12">
      ...
      <user negate="yes">fortishield|root</user>
      <description>Unexpected user successfully logged to the system.</description>
    </rule>

This rule will trigger when a user different from ``root`` or ``fortishield`` successfully login into the system.
...
+----------------------+------------------------+
| Docker integration   | Fortishield-Docker           |
+----------------------+------------------------+
...
Since Fortishield version 3.3 it is possible to include any decoded field (static or dynamic) to the description message. You can use the following syntax: ``$(field_name)`` to add a field to the description.
