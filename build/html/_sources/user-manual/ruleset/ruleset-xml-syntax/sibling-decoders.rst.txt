...
Our development team and the Fortishield community at large are constantly :doc:`contributing to the ruleset <../contribute>` but at the same time, new security-relevant devices and software programs are being created constantly all around the globe.
...
Fortishield's log analysis
^^^^^^^^^^^^^^^^^^^^

Firstly, it is of the utmost importance to understand how logs are analyzed by Fortishield's ``analysisd``. Fortishield collects log messages from a vast amount of sources relevant to an environment's security. Then, for every message ingested, the ruleset is analyzed using a very simple and resource-efficient logic that allows the Fortishield manager to handle large amounts of log data requiring few resources.
...
Using `/var/ossec/bin/fortishield-logtest` we get:
