...
.. meta::
  :description: Fortishield provides an automated way of building macOS packages. Learn how to build your own Fortishield macOS packages in this section of our documentation.
...
Fortishield provides an automated way of building macOS packages, keep in mind that to build an macOS package you must run this tool in an macOS system.
...
If ``Packages`` and ``Brew`` are not already installed in your system, they can be installed using the generate_fortishield_packages.sh script

Download our fortishield-packages repository from GitHub and go to the macos directory.

.. code-block:: console

  $ git clone https://github.com/fortishield/fortishield-packages && cd fortishield-packages/macos && git checkout v|FORTISHIELD_CURRENT_OSX|
...
.. code-block:: console
  ...
  # ./generate_fortishield_packages.sh -b v|FORTISHIELD_CURRENT_OSX| -s /tmp

This will build a |FORTISHIELD_CURRENT_OSX| Fortishield agent macOS  package and store it in ``/tmp``.

.. code-block:: console

  # ./generate_fortishield_packages.sh -b v|FORTISHIELD_CURRENT_OSX| -s /tmp -j 6

This will also build a |FORTISHIELD_CURRENT_OSX| Fortishield agent macOS package and store it in ``/tmp`` but will use 6 jobs to compile the sources.

.. code-block:: console

  # ./generate_fortishield_packages.sh -b v|FORTISHIELD_CURRENT_OSX| -s /tmp -j 6 -c
...
.. code-block:: console
  ...
  $ sudo ./generate_fortishield_packages.sh -b v|FORTISHIELD_CURRENT_OSX| -j 4 -r 1 --notarize \
...
The result of the notarization will be stored in fortishield-packages/macos/request_result.txt.
