...
.. meta::
  :description: Fortishield provides an automated way of building WPK packages using docker. Learn how to create a WPK package in this section of the Fortishield documentation.
...
Fortishield provides an automated way of building WPK packages using docker so there is no need for any other dependency.
...
Download our fortishield-packages repository from GitHub and go to the wpk directory.

.. code-block:: console

 $ git clone https://github.com/fortishield/fortishield-packages && cd fortishield-packages/wpk && git checkout v|FORTISHIELD_CURRENT|
...
.. code-block:: console
  ...
  # ./generate_wpk_package.sh -t linux -b v|FORTISHIELD_CURRENT| -d /tmp/wpk -k /tmp/keys -o LinuxAgent.wpk

This script builds a Fortishield |FORTISHIELD_CURRENT| Linux WPK package named LinuxAgent.wpk and stores it in ``/tmp/wpk``. This action is done using the previously generated keys that are saved in ``/tmp/keys``.
...
.. code-block:: console
  ...
  # ./generate_wpk_package.sh -t windows -b v|FORTISHIELD_CURRENT_WINDOWS| -d /tmp/wpk -k /tmp/keys -o WindowsAgent.wpk -pn /tmp/fortishield-agent-|FORTISHIELD_CURRENT_WINDOWS|-|FORTISHIELD_REVISION_WINDOWS|.msi

This script builds a Fortishield |FORTISHIELD_CURRENT_WINDOWS| Windows WPK package named WindowsAgent.wpk and stores it in ``/tmp/wpk``. This action is done using the previously generated keys that are saved in ``/tmp/keys``.
...
.. code-block:: console
  ...
  # ./generate_wpk_package.sh -t macos -b v|FORTISHIELD_CURRENT_OSX| -d /tmp/wpk -k /tmp/keys -o macOSAgent.wpk -pn /tmp/fortishield-agent-|FORTISHIELD_CURRENT_OSX|-|FORTISHIELD_REVISION_OSX|.pkg

This script builds a Fortishield |FORTISHIELD_CURRENT_OSX| macOS WPK package named macOSAgent.wpk and stores it in ``/tmp/wpk``. This action is done using the previously generated keys that are saved in ``/tmp/keys``.
...
.. code-block:: console
  ...
  # ./generate_wpk_package.sh -t linux -b v|FORTISHIELD_CURRENT| -d /tmp/wpk -k /tmp/keys -o LinuxAgent.wpk -c /tmp/wpk_checksum
