...
.. meta::
  :description: AWS CloudWatch Logs is a service that allows users to centralize the logs from all their systems. Learn how to use CloudWatch Logs with Fortishield here.
...
AWS CloudWatch logs can be accessed by using the Fortishield CloudWatch Logs integration. The AWS API allows Fortishield to retrieve those logs, analyze them, and raise alerts if applicable.
...
#. Open the Fortishield configuration file (``/var/ossec/etc/ossec.conf``) and add the following configuration block to enable the integration with CloudWatch Logs:
