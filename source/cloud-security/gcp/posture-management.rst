...
.. meta::
   :description: Use Fortishield to manage posture security on the Google Cloud Platform (GCP).
...
Fortishield is a free, open source, enterprise-grade security monitoring platform that provides comprehensive protection for cloud, on-premises, containerized, and virtualized environments. This section demonstrates how to use Fortishield to manage posture security on the Google Cloud Platform (GCP).

Integrating Fortishield with GCP
--------------------------

Fortishield integrates with GCP using the Google Cloud publisher and subscriber service (GCP Pub/Sub). Google Cloud Pub/Sub is a messaging service that helps you send and receive log data between applications. Fortishield provides an :doc:`integration module for GCP <index>` that fetches logs from the Pub/Sub service.
...
Create a new GCP project and a service account that enables the Fortishield GCP module to pull log data from the Google Pub/Sub service. Once this is done, configure the Pub/Sub and the Sink services. The Sink service routes cloud security posture logs from the central GCP Cloud Logging service to the Pub/Sub service. 
...
#. Go to the **IAM and admin** drop-down menu and select **Service accounts** to create a new service account. On the service accounts creation page, add the ``Pub/Sub Publisher`` and ``Pub/Sub Subscriber`` roles to the account.
   ...
   -  **Service account name** is the privileged account that Fortishield uses to connect to GCP.
   -  **Roles** are the rights given to the service account.

#. Open the newly created service account and create a _`private key in JSON format`. Your browser automatically downloads the key. Fortishield uses the key to authenticate to your GCP project.
...
Configure the Fortishield server to receive logs from GCP by performing the following steps.
...
#. Update the ``/var/ossec/wodles/gcloud/credentials.json`` file with the contents of the `private key in JSON format`_ file downloaded earlier. The Fortishield GCP module uses the key file to authenticate your GCP account.

#. Append the following content to the ``/var/ossec/etc/ossec.conf`` configuration file. The configuration specifies how Fortishield connects to GCP using the project ID, GCP PubSub subscription ID, and a credential.
...
#. Create a rule file ``gcp_posture.xml`` in the ``/var/ossec/etc/rules/`` directory and add the following custom rules to detect GCP posture findings:
   ...
   -  Rule ID ``100200`` is triggered when Fortishield detects a misconfiguration in a GCP account.
...
The Findings module is a GCP Security Command Centre service that records security misconfigurations across a GCP project. The simulation will produce sample misconfigurations that will be shipped to Fortishield.
...
.. thumbnail:: /images/cloud-security/gcp/gcp-posture-management-alerts.png
   :title: Fortishield alerts for the GCP posture management
   :alt: Fortishield alerts for the GCP posture management
   :align: center
   :width: 80%
