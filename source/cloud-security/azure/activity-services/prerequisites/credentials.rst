...
.. meta::
  :description: Learn what you need to provide access credentials to the Fortishield Azure module so it can successfully connect to Azure in this section of the Fortishield documentation.

Configuring Azure credentials
=============================

It is necessary to provide access credentials to the Fortishield Azure module so it can successfully connect to Azure. The credentials required vary depending on the type of monitoring.
...
Another authentication option is to set up credentials by storing them directly into the Fortishield configuration file ``/var/ossec/etc/ossec.conf``, inside of the ``<graph>``, ``<log_analytics>`` and ``<storage>`` blocks on the module configuration.
