...
Your environment is configured by default to send Fortishield output files to archive data.


There are two Fortishield output files in JSON format:

- ``/var/ossec/logs/archives/archives.json``: If  you set ``logall_json`` to ``yes``, Fortishield stores all events in this file and sends it to archive data, regardless of whether they triggered an alert.
