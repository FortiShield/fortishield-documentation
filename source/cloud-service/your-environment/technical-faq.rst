...
.. meta::
  :description: Get answers to the most frequently asked questions about the Fortishield Cloud in this technical FAQ and get the most out of the Fortishield Cloud service.  
...
- `Can I send data directly to the Fortishield indexer of my environment?`_

- `Can I integrate with my Single Sign-On (SSO) method (LDAP, Okta, Active Directories)?`_

- `Do I have access to Fortishield API?`_

- `Do I have access to Fortishield indexer API?`_
...
All the communications are performed through Fortishield agents once they are registered to the environment.
  
Is it possible to change the URL to access the environment?
-----------------------------------------------------------

It is possible to get a new URL by opening a support ticket through the **Help** section on the Fortishield Cloud Console, but the previous URL is also kept.
...
You can upgrade or downgrade a setting by contacting the Fortishield team through the **Help** section of your Fortishield Cloud Console. See also :ref:`Adjusting environment settings <cloud_settings_adjust>`.
...
If the data ingestion is exceeded, events start to queue. If the queue becomes full, Fortishield discards the incoming events, which might lead to event loss. The cloud service automatically manages the queuing mechanism, ensuring optimal resource usage.

How do I get SSH access to my environment?
------------------------------------------

SSH access is not allowed for security reasons. Environments are managed from the Fortishield Cloud Console and Fortishield WUI.

How can I update my environment?
--------------------------------

Fortishield takes care of the updates, so your environment gets the latest version of Fortishield with no downtime.
 
Can I send syslog data directly to the environment?
---------------------------------------------------

No, all the communications are performed through Fortishield agents once they are registered into the environment. However, you have alternative options. For more information on how to forward syslog events to your environment, see the :ref:`Forward syslog events <cloud_your_environment_send_syslog>` section.

Can I send data directly to the Fortishield indexer of my environment?
----------------------------------------------------------------

No, all the communications are performed through Fortishield agents.

Can I integrate with my Single Sign-On (SSO) method (LDAP, Okta, Active Directories)?
----------------------------------------------------------------------------------------

Yes, you can access the Fortishield WUI of your environment through your SSO tool. To perform this action, you need to contact the Fortishield Support team through the **Help** section of your Fortishield Cloud Console.


Do I have access to Fortishield API?
------------------------------

You have access to the **Dev tools** through your Fortishield dashboard, where you can use the API. The Fortishield API is not exposed, but you can contact the Fortishield team through the **Help** section of your Fortishield Cloud Console to allow Fortishield API access from a specific IP address.


Do I have access to Fortishield indexer API?
--------------------------------------

The Fortishield indexer API is not accessible by default. If you want to access it, contact the Fortishield team through the **Help** section of your Fortishield Cloud Console to authorize the connection from a specific IP address. After authorization is granted, you have access to the ``GET`` methods of the Fortishield indexer API.
