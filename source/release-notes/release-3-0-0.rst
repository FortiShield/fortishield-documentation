...
.. meta::
  :description: Fortishield 3.0.0 has been released. Check out our release notes to discover the changes and additions of this release.
...
This section shows the most relevant new features of Fortishield v3.0.0. You will find more detailed information in our `changelog <https://github.com/fortishield/fortishield/blob/v3.0.0/CHANGELOG.md>`_ file.

For deploying your Fortishield environment see the :doc:`Installation guide <../installation-guide/index>`.

**New features:**

- `Grouping agents`_
- `Remote agent upgrades`_
- `Fortishield cluster for managers`_
...
Support for the grouping of agents has now been included at the Fortishield manager level, which makes centralized configuration more flexible and efficient.
...
This groups management feature is available via terminal using a CLI included in the Fortishield manager, as well using Fortishield API requests.
...
send a compressed and signed WPK (Fortishield Signed Package) that contains the necessary binaries and instructions to upgrade the agent.
...
WPK files will be generated by Fortishield for every new release. You may also use your own custom WPK files.
...
Fortishield cluster for managers
--------------------------

The Fortishield cluster provides new capability to scale Fortishield horizontally by adding as many manager nodes as needed to process events from the reporting agents.
...
More information on this new functionality can be found in the dedicated section at :ref:`Cluster basics <fortishield-cluster-introduction>`.

Automatic decoding for JSON events
----------------------------------

The Fortishield manager now includes a native decoder for the JSON format which can read any JSON event and extract its fields dynamically. This new decoder enables Fortishield to use all JSON fields/values for creating rules.
...
The integration with VirusTotal as a threat intelligence source, along with the existing FIM capabilities is a significant improvement in Fortishield's malware detection.
...
The procedure for using the MSI installer can be found at: :ref:`Install Fortishield agent on Windows <fortishield_agent_package_windows>`


Fortishield API
---------

The Fortishield API now includes functionality to manage all the features included in this release, such as:

- the management of remote agent upgrades,
- the requests for managing groups, and
- the management of the new Fortishield Cluster.

In addition, more new features can be found in the `API changelog <https://github.com/fortishield/fortishield-api/blob/3.0/CHANGELOG.md>`_.
...
For details on changes in the Ruleset, please visit the `Ruleset changelog <https://github.com/fortishield/fortishield-ruleset/blob/3.0/CHANGELOG.md>`_.
...
Additional features have been added to Fortishield 3.0.0 in order to improve its performance, including, but not limited to:
