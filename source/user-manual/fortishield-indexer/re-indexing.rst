...
To re-index an existing index, perform the following steps on either the Fortishield dashboard or the Fortishield server.
...
Run the following command on any Fortishield central component that is allowed to authenticate to the Fortishield API. Replace ``<INDEXER_USERNAME>`` and ``<INDEXER_PASSWORD>`` with the indexer username and password:
