...
.. meta::
    :description: Learn more about the agent management in Fortishield. In this section, we will show you how to group agents and how to manage multiple groups.
...
1. Once an agent has been added and connected to the manager, assign it to a group using the :doc:`agent_groups <../reference/tools/agent-groups>` tool or the
   Fortishield API. Below are examples of how to assign an agent with ID 002 to the group *'dbms'* using these methods:
   ...
   Using the **Fortishield API** endpoint :api-ref:`PUT /agents/{agent_id}/group/{group_id} <operation/api.controllers.agent_controller.put_agent_single_group>`:
   ...
   Using the **Fortishield API** endpoint :api-ref:`GET /groups/{group_id}/agents <operation/api.controllers.agent_controller.get_agents_in_group>`:
...
Since Fortishield v3.7.0, agents have the ability to belong to multiple groups. The agents will receive all the configuration files from each group. Configuration received from the last assigned group
...
This capability is focused to customize the agents' configuration with a higher level of granularity. The **Fortishield API** and **agent_groups**
...
In this example, the agent 001 has been added to `webserver` and `apache` groups. First of all, using the **Fortishield API** endpoint :api-ref:`PUT /agents/{agent_id}/group/{group_id} <operation/api.controllers.agent_controller.put_agent_single_group>`:
...
After that, we can ask about groups to which an agent belongs using the **Fortishield API** endpoint :api-ref:`GET /agents <operation/api.controllers.agent_controller.get_agents>`:
...
Finally, to check the synchronization status of the group configuration for agents, both following methods are available, **agent_groups** and **Fortishield API** endpoint :api-ref:`GET /agents <operation/api.controllers.agent_controller.get_agents>`:
...
The rest of the capabilities of **agent_groups** can be found at its :doc:`reference section <../reference/tools/agent-groups>`. The same for the :doc:`Fortishield API <../api/reference>`, which offers calls with similar behavior.
