...
.. meta::
    :description: Check out how to remove agents using the Fortishield API. The Fortishield API is an open source RESTful API that allows for interaction with the Fortishield manager.
    
.. _restful-api-remove:

Remove agents using the Fortishield API
----------------------------------
...
The examples use :ref:`an authentication token <api_log_in>`. To get your token, replace ``<user>:<password>`` with your Fortishield API credentials and run the following command:
