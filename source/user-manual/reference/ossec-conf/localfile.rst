...
.. meta::
  :description: Learn more about how to configure the collection of log data from files, Windows events, and from the output of commands with Fortishield.
...
Wildcards can be used on Linux and Windows systems, if the log file doesn't exist at ``fortishield-logcollector`` start time, such log will be re-scanned after ``logcollector.vcheck_files`` seconds.
...
.. code-block:: xml
  ...
  <localfile>
      <location>C:\Users\fortishield\myapp\*</location>
...
It allows to read new log content since ``fortishield-logcollector`` was stopped.

By default, when ``fortishield-logcollector`` is started it reads the logs generated since that moment.
Set it to ``no`` to collect events generated since ``fortishield-logcollector`` was stopped.
...
.. note::
  If the log rotates while ``fortishield-logcollector`` is stopped and ``only-future-events`` is set to ``no``, it will start reading from the beginning of the log.
...
This label can be used to filter *Windows* ``eventchannel`` events or *macOS* ULS logs (``macos``) that Fortishield will process.
...
Sample Log message as analyzed by fortishield-analysisd:
