...
.. meta::
  :description: The Fortishield GCP Pub/Sub module allows you to pull log data from Google Pub/Sub. Learn more about how to configure the module in this section.
...
For example ``<project_id>fortishield-dev</project_id>``.
...
For example ``<subscription_name>fortishield-name</subscription_name>``.

credentials_file
^^^^^^^^^^^^^^^^

Path to the Google Cloud credentials file. It can be absolute path or relative to ``FORTISHIELD_HOME``.
...
This option has no effect. The module now uses the :ref:`fortishield_modules.debug <fortishield_modules_options>` level to set its logging level.
...
Pull logs on Fortishield agent start or restart.
...
.. code-block:: xml
    ...
    <gcp-pubsub>
        ...
        <project_id>fortishield-dev</project_id>
        <subscription_name>fortishielddns</subscription_name>
