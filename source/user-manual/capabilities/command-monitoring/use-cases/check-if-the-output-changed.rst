...
For this endpoint, we configure Fortishield to monitor the output of the Linux ``netstat`` command and alert when a change is detected.
...
Fortishield has an out-of-the-box rule with ID ``533`` that generates an alert when there is a change in the ``netstat`` listening ports. The rule is defined below and is found in the `Fortishield GitHub repository <https://github.com/fortishield/fortishield-ruleset/blob/master/rules/0015-ossec_rules.xml>`__.
...
Go to **Threat Hunting** module on the Fortishield dashboard to visualize the alert showing the changes in the network.
