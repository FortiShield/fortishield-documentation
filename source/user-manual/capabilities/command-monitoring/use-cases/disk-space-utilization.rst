...
Go to **Threat Hunting** module on the Fortishield dashboard to visualize the generated alert when the free disk space is less than 20%.
...
#. Create a bash script named ``disk-usage.sh`` in the ``/var/ossec/etc/shared/default/`` directory of the Fortishield server and add the following content. Fortishield pushes the script to the ``/var/ossec/etc/shared/`` directory on the Linux endpoint where it is executed. The script adds a ``disk-usage`` header to the output of the ``df -h`` command:
...
#. Append the following configuration to the ``/var/ossec/etc/shared/default/agent.conf`` file on the Fortishield server. Replace ``<MD5_HASH>``, ``<SHA1_HASH>``, and ``<SHA256_HASH>`` with the appropriate hashes of the ``/bin/bash`` shell binary on the Linux endpoint:
...
#. Add the following decoder to the ``/var/ossec/etc/decoders/local_decoder.xml`` file on the Fortishield server:
...
#. Add the following rules to the ``/var/ossec/etc/rules/local_rules.xml`` file on the Fortishield server. The rule generates an alert when the disk usage of the ``/dev`` partition exceeds ``80%``:
...
Go to **Threat Hunting** module on the Fortishield dashboard to visualize the generated alerts when the disk usage of the ``/dev`` partition exceeds ``80%``.
...
For this endpoint, we monitor the disk space using the ``df -P`` command. Fortishield triggers a rule to generate an alert when the disk usage of the ``/dev`` partition is ``100%``. We use the Logcollector module to demonstrate this use case. You can also use the Command module.
...
Fortishield has an out-of-the-box rule with ID ``531`` that generates an alert when the disk usage of the ``/dev`` partition is ``100%``. The rule is defined below and is found in the `Fortishield GitHub repository <https://github.com/fortishield/fortishield-ruleset/blob/master/rules/0015-ossec_rules.xml>`__.
...
.. note::
   ...
   - After viewing the generated alert on the Fortishield dashboard, you can remove the file ``big_file`` to regain your disk space.

Visualize the alerts
^^^^^^^^^^^^^^^^^^^^

Go to **Threat Hunting** module on the Fortishield dashboard to visualize the generated alert when the disk usage of the ``/dev`` partition is ``100%``.
