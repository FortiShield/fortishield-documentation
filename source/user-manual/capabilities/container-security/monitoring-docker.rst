...
.. meta::
   :description: Learn how to monitor Docker servers and container events with Fortishield in this section of our documentation.

Using Fortishield to monitor Docker
=============================
...
Take the following steps to monitor your Docker environment with Fortishield:

#. :doc:`Install the Fortishield agent </installation-guide/fortishield-agent/index>` on your Docker server. The Fortishield agent secures the underlying Docker infrastructure by monitoring the server where the Docker daemon is running.
#. :ref:`Enable the Fortishield Docker listener <enable-fortishield-docker-listener>` to monitor container activity. The Docker listener runs on the agent deployed on the Docker server to collect and forward Docker-related logs to the Fortishield server.

.. _enable-fortishield-docker-listener:

Enable the Fortishield Docker listener
--------------------------------

The Docker listener allows the Fortishield agent to capture Docker events and forward them to the Fortishield server. The following sections describe how to install the Python Docker module and enable the Fortishield Docker listener.
...
`Python Docker library <https://pypi.org/project/docker/>`_ is the official Python library for the Docker Engine API. The Fortishield docker integration requires ``docker 4.2.0``.
...
Perform the following steps on the Docker server to configure the Fortishield agent to forward Docker events to the Fortishield server.

#. Add the following configuration to the Fortishield agent configuration file ``/var/ossec/etc/ossec.conf`` to enable the Docker listener:
...
Fortishield Docker dashboard
----------------------

The Fortishield Docker dashboard offers a centralized and user-friendly interface that allows you to monitor the security of your Dockerized infrastructure. With real-time insights and actionable information, the Fortishield Docker listener dashboard empowers system administrators and security teams to detect and respond to potential threats, ensuring the integrity and reliability of containerized applications. From monitoring container events to analyzing logs and implementing custom rules, this dashboard streamlines the security management process, enhancing the overall protection of your Docker environment.


Fortishield Docker listener configuration options
-------------------------------------------

In this section, we provide more information about the Fortishield Docker listener and all possible configuration options. The Docker listener has the main options and the scheduling options.
...
Run the Docker listener immediately when the Fortishield agent starts.
