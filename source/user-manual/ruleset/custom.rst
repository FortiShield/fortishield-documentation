...
.. meta::
  :description: Check out how to add new decoders and rules, and how to change existing ones in this section of the Fortishield documentation. 
...
Customize the Fortishield ruleset to fit your needs and enhance detection capabilities. To achieve this, you can:
...
#. Run ``/var/ossec/bin/fortishield-logtest``. Input the example log above to test the decoder and rule:
   ...
   To test your rules and decoders using ``fortishield-logtest``, it's enough to save the changes made to the decoder and rule files. However, you need to restart the Fortishield manager to generate alerts based on these changes.  

#. Restart the Fortishield manager to load the updated rules and decoders:
...
You can change the default Fortishield rules. To do so, we recommend copying the rules to a file in the ``/var/ossec/etc/rules/`` directory, making the necessary changes, and adding the ``overwrite="yes"`` tag to the modified rules. These steps guarantee that your changes won't be lost during updates.
...
#. Restart the Fortishield manager to load the updated rules:
...
#. Edit the ``/var/ossec/etc/ossec.conf`` configuration file. Set the ``<decoder_exclude>`` tag to exclude the original ``ruleset/decoders/0310-ssh_decoders.xml`` decoder file from the loading list. With this configuration, Fortishield loads the decoder file located in the user folder and not the file in the default folder.
...
#. Restart the Fortishield manager so the changes take effect:
