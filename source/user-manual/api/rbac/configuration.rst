...
.. meta::
  :description: Check out how to configure the Role-Based Access Control (RBAC) with Fortishield in this step-by-step guide of our documentation. 
...
=============
.. note::
    It is necessary to log into the Fortishield API before attempting to add or modify any RBAC configuration, such as policies, roles ,users, or security rules. A detailed guide on how to log in can be found within the :ref:`Getting started <api_getting-started>` section of the Fortishield API.

Set RBAC mode
-------------
As explained in the :doc:`How it works </user-manual/api/rbac/how-it-works>` section, it is possible to modify the RBAC mode and change it to ``white`` or ``black``. This can be done using the :api-ref:`PUT /security/config <operation/api.controllers.security_controller.put_security_config>` Fortishield API endpoint. It can also be restored to default using :api-ref:`DELETE /security/config <operation/api.controllers.security_controller.delete_security_config>`.
...
The Fortishield API request for this particular example would be as follows:
...
The Fortishield API response will be something similar to this. The highlighted ID should be used later on to assign the policy to the role:
...
Use the following Fortishield API endpoint :api-ref:`GET /security/users/me/policies <operation/api.controllers.security_controller.get_user_me_policies>` to obtain the final processed policies for the currently logged in user:
