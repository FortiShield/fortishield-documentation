...
.. meta::
  :description: Fortishield 3.8.0 has been released. Check out our release notes to discover the changes and additions of this release.
...
- `fortishield/fortishield <https://github.com/fortishield/fortishield/blob/v3.8.0/CHANGELOG.md>`_
- `fortishield/fortishield-api <https://github.com/fortishield/fortishield-api/blob/v3.8.0/CHANGELOG.md>`_
- `fortishield/fortishield-ruleset <https://github.com/fortishield/fortishield-ruleset/blob/v3.8.0/CHANGELOG.md>`_
- `fortishield/fortishield-kibana-app <https://github.com/fortishield/fortishield-kibana-app/blob/v3.8.0-6.5.4/CHANGELOG.md>`_
- `fortishield/fortishield-splunk <https://github.com/fortishield/fortishield-splunk/blob/v3.8.0-7.2.3/CHANGELOG.md>`_
...
The alerts that Fortishield sends to Elasticsearch are now including all these AWS additions, so you can track all your AWS services / buckets using Kibana.
...
- Windows events are now being fetched in JSON format, which provides a useful format for third-party software and makes Fortishield be more optimized while analyzing Windows events. This improves Windows alerts result and analyzing performance. From this version, Windows agents apply a new rule set
...
- Who-data module is now able to re-connect to an Audit socket even if the instance is using enforcing with SELinux. Before this enhancement, Fortishield could not re-connect the socket after restart Fortishield if enforcing was being used along Audit.
...
**Other Fortishield core fixes and improvements**

- When upgrading, databases used for FIM purposes are now auto-upgraded by Fortishield (no need for scripts).
- Vulnerability detector has been improved for RedHat systems.
- This version also fixes some known issues when using Fortishield on ARM, HP-UX or AIX systems.
...
**Fortishield API also has these fixes for this version**

- Now the Fortishield API service gets the group ID and user ID properly when using Docker containers.
- Added missing information when requesting certain files from a group.
- Rule variables from the Fortishield ruleset are now replaced by its real value when fetching rules.
...
- Edit group configuration (agent.conf), just open the XML editor we've added, edit the group configuration and send it to the Fortishield API.
...
**Modify the Fortishield monitoring index pattern name**

This was added before for Fortishield alerts indices, now you can do the same for monitoring indices editing the app configuration file (config.yml).

.. code-block:: yaml

  # Default index pattern to use for Fortishield monitoring
  fortishield.monitoring.pattern: fortishield-monitoring-3.x-*
...
- Now the app looks for the request target if the destination is unreachable. Now you'll know if it was Elasticsearch or the Fortishield API.
