...
.. meta::
   :description: Fortishield helps meet the processing integrity PI1.4 requirement by monitoring and reporting file changes using the FIM module.
...
Use case: Detecting file changes using the Fortishield File Integrity Monitoring module
---------------------------------------------------------------------------------

This use case shows how Fortishield helps meet the *processing integrity PI1.4* requirement by monitoring and reporting file changes using the FIM module. In this scenario, we show how you can configure the Fortishield agent on a Ubuntu 22.04 endpoint to detect changes in the ``critical_folder`` directory.
...
#. Add the highlighted configuration to the ``<syscheck>`` block of the Fortishield agent configuration file ``/var/ossec/etc/ossec.conf``:
...
#. Select **TSC** from the Fortishield dashboard to view the alert with rule ID ``550``.
