...
In this use case, we configure a CentOS 7 endpoint to forward logs using rsyslog to the Fortishield server for analysis. On the CentOS 7 endpoint, we create and delete the user account ``Stephen``. Fortishield has default rules that generate alerts for the creation and deletion of user accounts.

CentOS endpoint
^^^^^^^^^^^^^^^

Perform the following steps to forward logs using rsyslog to the Fortishield server.
...
Perform the following steps to configure the Fortishield server to receive logs from the CentOS 7 endpoint.
...
#. Restart the Fortishield manager for the changes to take effect:
...
Navigate to **Threat Hunting** module on the Fortishield dashboard to view the alerts.   
...
By default, the Fortishield agent monitors the installation of applications using the configuration below located in the Fortishield agent configuration file ``C:\Program Files (x86)\ossec-agent\ossec.conf``:
...
Fortishield has a built-in rule ``60612`` to detect when an application is installed on a Windows endpoint. You can view this rule in the ``/var/ossec/ruleset/rules/0585-win-application_rules.xml`` file on the Fortishield server.
...
After installing Dr. Memory, navigate to **Threat Hunting** on the Fortishield dashboard and apply the filter ``rule.id:60612`` to view the alert.
...
In this use case, we configure Fortishield to detect when PowerShell adds a new Windows registry key. 

Windows endpoint
^^^^^^^^^^^^^^^^
Perform the following steps to enable PowerShell logging on a Windows endpoint and configure the Fortishield agent to monitor logged PowerShell activities.
...
#. Add the following in between the ``<ossec_config>`` tags of the Fortishield agent configuration file ``C:\Program Files (x86)\ossec-agent\ossec.conf`` to monitor PowerShell logs:
...
#. Restart the Fortishield agent via PowerShell with administrator privileges to apply the change:
...
Fortishield has a built-in rule ``91843`` to detect when a PowerShell adds a new Windows registry key. You can view this rule in the ``/var/ossec/ruleset/rules/0915-win-powershell_rules.xml`` file on the Fortishield server:
...
#. Navigate to **Threat Hunting** on the Fortishield dashboard and apply the ``rule.id:91843`` filter to view the alert.
