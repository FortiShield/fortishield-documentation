...
.. meta::
  :description: The Fortishield FIM module scans the Windows Registry periodically and triggers an alert when it detects changes in the entries. Learn more about it in this section.
...
The Fortishield FIM module scans the Windows Registry periodically and triggers an alert when it detects changes in the entries.

How it works
------------

The FIM module runs periodic scans of monitored Windows Registry entries and stores their checksums and other attributes in a local FIM database. You can specify which registry entries to monitor in the configuration of the Fortishield agent.

Upon a scan, the Fortishield agent reports any changes the FIM module finds in the monitored registry entries to the Fortishield server. The FIM module looks for file modifications by comparing the checksums of a registry entry to its stored checksums and attribute values. It generates an alert if it finds discrepancies.

The Fortishield FIM module uses two databases to collect FIM event data, such as registry entry creation, modification, and deletion data. One is a local SQLite-based database on the monitored endpoint that stores the data in  ``C:\Program Files (x86)\ossec-agent\queue\fim\db``. The other is an agent database on the Fortishield server stored at ``/var/ossec/queue/db``.
...
The FIM module synchronization mechanism ensures synchronization between the Fortishield agent and the Fortishield server databases. It always updates the file inventory in the Fortishield server with the data available to  the Fortishield agent. This allows for servicing FIM-related API queries regarding the Fortishield agents.
...
.. note::
   ...
   Registry keys matching your configuration might be created after the initial FIM scan. Fortishield scans these new keys only from the next scheduled FIM scan.
...
#. After configuring these attributes, restart the Fortishield agent using PowerShell with administrator privileges to apply the changes:
...
#. After setting the recursion level, restart the Fortishield agent to apply the configuration:
...
You must use the ``report_changes`` attribute with caution. Fortishield copies every single monitored file to a ``C:\Program Files (x86)\ossec-agent\queue\diff\registry`` and this increases storage usage.
...
#. Restart the Fortishield agent to apply the configuration:
...
Navigate to **File Integrity Monitoring** on the Fortishield dashboard to view the alert generated when the FIM module detects a modification of the monitored registry value.
...
#. Add this configuration to the ``C:\Program Files (x86)\ossec-agent\ossec.conf`` configuration file of the Fortishield agent:
...
With the Fortishield FIM module, you can detect any suspicious or unknown programs added to the startup registry keys. This allows you to take appropriate action to remove them before they cause harm to your system.
...
Fortishield monitors the startup registry keys automatically, out-of-the-box, without requiring any user special action or configuration. By default, the Fortishield agent configuration file at ``C:\Program Files (x86)\ossec-agent\ossec.conf`` uses the following setting to monitor the startup registry keys:
...
Navigate to **File Integrity Monitoring** on the Fortishield dashboard to view the alert generated when the FIM module detects changes in the Windows startup registries.
