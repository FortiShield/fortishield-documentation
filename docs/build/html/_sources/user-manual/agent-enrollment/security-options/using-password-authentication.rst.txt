...
This method requires a password during the enrollment process to ensure that agents enrolled with the Fortishield manager are authenticated.
...
Before an agent can be enrolled to the Fortishield manager using the password authentication method, the following must be done on the Fortishield manager:
   ...
   #. **Recommended** - Setting your own password. This is done by creating the file ``/var/ossec/etc/authd.pass`` on the manager with your password.
      ...
      #. Change the ``authd.pass`` file permissions and ownership.
         ...
         .. code-block:: console
            ...
            # chown root:fortishield /var/ossec/etc/authd.pass

      #. Restart the Fortishield service for the changes to take effect.

         .. include:: /_templates/common/restart_manager.rst


   #. Allowing the enrollment service to set a random password. A new random password is generated each time the Fortishield manager service is restarted. 
      ...
      #. Run the following command to get the agent enrollment password: 
         ...
         .. code-block:: xml
            ...
            2022/01/11 12:41:35 fortishield-authd: INFO: Accepting connections on port 1515. Random password chosen for agent authentication: 6258b4eb21550e4f182a08c10d94585e
...
#. Create the file ``/var/ossec/etc/authd.pass`` with the enrollment password in it.
   ...
   #. File permissions for the ``authd.pass`` file should be set to 640 and the owner should be ``root``. The permissions and ownership can be configured by running the commands below:
      ...
      .. code-block:: xml
         ...
         -rw-r--r-- 1 root fortishield 18 Jan 11 13:03 /var/ossec/etc/authd.pass

#. Add the Fortishield manager IP address or DNS name in the ``<client><server><address>`` section of the agent configuration file ``/var/ossec/etc/ossec.conf``.
...
#. Add the Fortishield manager IP address or DNS name in the ``<client><server><address>`` section of ``C:\Program Files (x86)\ossec-agent\ossec.conf``:
...
#. Create a file called ``/Library/Ossec/etc/authd.pass`` and save the password to it.
   ...
   #. File permissions for the ``authd.pass`` file should be set to 640 and the owner should be root. The permissions and ownership can be configured by running the commands below:
      ...
      .. code-block:: console 
         ...
         # chown root:fortishield /Library/Ossec/etc/authd.pass
      ...
      .. code-block:: xml
         ...
         -rw-r--r-- 1 root fortishield 18 Jan 11 13:03 /Library/Ossec/etc/authd.pass

#. Add the Fortishield manager IP address or DNS name in the ``<client><server><address>`` section of ``/Library/Ossec/etc/ossec.conf``:
