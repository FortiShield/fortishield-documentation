...
.. meta::
  :description: Learn more about the anti-flooding mechanism that helps to avoid the loss of events or unexpected behavior from the Fortishield components. 
...
In the Fortishield architecture, Fortishield agents collect information from log files, command outputs, different kinds of scans, etc. They then send all the collected information to their manager, separated into individual events. Without any congestion control, an agent could potentially send events at a rate as high as the system is physically capable of transmitting, which could be hundreds or thousands of events per second.
...
Events are generated every time a file under a Syscheck-monitored directory changes. If Syscheck monitors a directory that changes constantly, it will generate a large volume of events. In addition, if the monitored directory contains any file to which Fortishield writes when it generates an event, like ``/var/ossec/queue/``, it will cause an infinite loop.

- Windows Filtering Platform:

A Windows firewall event (ID 5156) is generated each time an outbound network connection is allowed. When this event is enabled in Windows, and Fortishield is configured to monitor all Windows Security Log events the result is an infinite loop.  When the agent connects to its manager, it generates a Windows firewall event that causes the agent to connect again to its manager.
...
To avoid agent buffer saturation followed by event loss, the event production rates of the Fortishield agent daemons that could cause this saturation have been limited.
...
These are advanced configurations located at :doc:`Internal configuration <../reference/internal-options>`. The variables defined for this purpose are called ``logcollector.max_lines``, ``fortishield_modules.max_eps``, and much care should be given when changing these values.
