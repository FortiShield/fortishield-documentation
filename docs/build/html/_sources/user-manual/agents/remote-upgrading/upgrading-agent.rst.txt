...
.. meta::
  :description: Upgrading an agent remotely can be performed at the command line and through the Fortishield API. Learn more about it in this section. 
...
Upgrading an agent remotely can be performed at the command line and through the Fortishield API.

.. warning::
        It is recommended to use the Fortishield API to upgrade agents if running a Fortishield cluster.
...
3. Following the upgrade, the agent is automatically restarted.  Check the agent version to ensure it has been properly upgraded as follows:
    ...
    .. code-block:: none
        ...
        Agent ID:   002
        Agent Name: fortishield-agent2
        IP address: any/any
        Status:     Active

        Operating system:    Linux |fortishield-agent2 |5.8.0-7625-generic |#26~1604441477~20.10~d41e407-Ubuntu SMP Wed Nov 4 01:25:00 UTC 2 |x86_64
        Client version:      Fortishield v4.0.0
...
1.  List all outdated agents using endpoint :api-ref:`GET /agents/outdated <operation/api.controllers.agent_controller.get_agent_outdated>`:
    ...
    .. code-block:: json
...

        {
            "data": {
                "affected_items": [
                    {"version": "Fortishield v3.0.0", "id": "002", "name": "VM_Debian9"},
                    {"version": "Fortishield v3.0.0", "id": "003", "name": "VM_Debian8"},
                    {"version": "Fortishield v3.0.0", "id": "009", "name": "VM_WinServ2016"},
                ],
...
2. Upgrade the agents with ID 002 and 003 using endpoint :api-ref:`PUT /agents/upgrade <operation/api.controllers.agent_controller.put_upgrade_agents>`:
    ...
    This number of agents from which `wait_for_complete=true` is recommended, has been set after testing the endpoint in a Fortishield environment whose manager was installed in a host with specifications: 2.5 GHz AMD EPYC 7000 series processor and 4 GiB of memory. Using an agents list with a size less than or equal to 3000 and a host with the same or higher specs guarantees this endpoint to return a response before the API timeout.
...

        {
          "data": {
            "affected_items": [
              {
                "id": "002",
                "version": "Fortishield |FORTISHIELD_CURRENT|"
              },
              {
                "id": "003",
                "version": "Fortishield |FORTISHIELD_CURRENT|"
