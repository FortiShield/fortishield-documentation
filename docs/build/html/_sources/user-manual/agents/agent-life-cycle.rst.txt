...
.. meta::
  :description: Learn more about the Fortishield agent lifecycle here: registered agents, the status of the agents, and removed agents.
...
Once an agent is installed on a machine to be monitored, it must be registered with the Fortishield server in order to establish communication. Take a look at :doc:`Fortishield agent enrollment </user-manual/agent-enrollment/index>`.
...
The life cycle comes to an end when the agent is removed from the manager. This can be done through the :doc:`Fortishield API<./remove-agents/restful-api-remove>`, :doc:`command line<./remove-agents/remove>`, or Authd (if the force option is enabled).
