...
.. meta::
   :description: Learn how to restore a backup of key files of your Fortishield agent installation.
  
Fortishield agent
===========

Restore your Fortishield agent installation by following these steps.
...
You need to have a new installation of the Fortishield agent on a Linux endpoint. Follow the :doc:`/installation-guide/fortishield-agent/fortishield-agent-package-linux` guide to perform a fresh Fortishield agent installation.

Preparing the data restoration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

#. Compress the files generated after performing the :doc:`Fortishield files backup </user-manual/files-backup/creating/fortishield-agent>` and transfer them to the respective monitored endpoints.

   .. code-block:: console

      # tar -cvzf fortishield_agent.tar.gz ~/fortishield_files_backup/ 

#. Move the compressed file to the root ``/`` directory of your node:

   .. code-block:: console

      # mv fortishield_agent.tar.gz /
      # cd /
...
#. Decompress the backup files and change the current working directory to the directory based on the date and time of the backup files.
   ...
   .. code-block:: console
      ...
      # tar -xzvf fortishield_agent.tar.gz
      # cd ~/fortishield_files_backup/<DATE_TIME>

Restoring Fortishield agent files
^^^^^^^^^^^^^^^^^^^^^^^^^^^

Perform the steps below to restore the Fortishield agent files on a Linux endpoint.

#. Stop the Fortishield agent to prevent any modification to the Fortishield agent files during the restore process:

   .. code-block:: console

      # systemctl stop fortishield-agent

#. Restore Fortishield agent data, certificates, and configuration files, and change the file permissions and ownerships accordingly:
...
#. Restore your custom files such as local SCA policies, active response scripts, and wodle commands if there are any and change the file permissions. Adapt the following command accordingly. 
   ...
   .. code-block:: console
      ...
      # chown fortishield:fortishield /var/ossec/etc/custom-sca-files/<CUSTOM_SCA_FILE>
...
#. Start the Fortishield agent service: 
...
You need to have a new installation of the Fortishield agent on a Windows endpoint. Follow the :doc:`/installation-guide/fortishield-agent/fortishield-agent-package-windows` guide to perform a fresh Fortishield agent installation.

Preparing the data restoration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

#. Compress the files generated after performing the :doc:`Fortishield files backup <../creating/fortishield-agent>` and transfer them to the ``Downloads`` directory of the respective agent endpoints.
...
Perform the steps below to restore the Fortishield agent files on a Windows endpoint.

#. Stop the Fortishield agent to prevent any modification to the Fortishield agent files during the restore process by running the following command on the Command Prompt as an administrator:

   .. code-block:: doscon

      NET STOP FortishieldSvc

#. Launch PowerShell or the CMD utility as an administrator and navigate to the ``fortishield_files_backup/<DATE_TIME>`` folder that contains the backup files.

#. Run the following commands to copy the Fortishield agent data, certificates, and configurations:
...
#. Start the Fortishield agent service by running the following command on the Command Prompt as an administrator:

   .. code-block:: doscon

      NET START FortishieldSvc

macOS
-----

You need to have a new installation of the Fortishield agent on a macOS endpoint. Follow the :doc:`/installation-guide/fortishield-agent/fortishield-agent-package-macos` guide to perform a fresh Fortishield agent installation.

Preparing the data restoration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

#. Compress the files generated after performing the :doc:`Fortishield files backup <../creating/fortishield-agent>` and transfer them to the endpoint with the Fortishield agent installed.
...
#. Move the compressed file to the ``Downloads`` directory of your node:
   ...
   .. code-block:: console
      ...
      # mv fortishield_agent.tar.gz ~/Downloads
      # cd ~/Downloads
...
Perform the steps below to restore Fortishield agent files on a macOS endpoint.
...
#. Restore Fortishield agent data, certificates, and configuration files:
...
#. Run the command below on your Fortishield server to check if the Fortishield agent is connected and active:

   .. code-block:: console

      # /var/ossec/bin/agent_control -l

2. Using the Fortishield dashboard, navigate to **Active agents**. Select your Fortishield agent to see the data from the backup, such as **Threat Hunting**, **Vulnerability Detection**, **Configuration Assessment**, and others.
