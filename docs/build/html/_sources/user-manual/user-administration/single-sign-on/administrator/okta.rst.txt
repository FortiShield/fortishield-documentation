...
.. meta::
   :description: Okta Inc. provides technologies that enable secure user authentication into applications. Learn more about it and the administrator role in this section of the Fortishield documentation.

Okta
====

`Okta Inc. <https://www.okta.com/>`_ is an identity and access management company that provides technologies that enable secure user authentication into applications. In this guide, we integrate the Okta IdP to authenticate users into the Fortishield platform.
...
#. Create a new group. Navigate to **Directory** > **Groups** and add a group.
    ...
    Create a new group using any name. In our case, we name it ``fortishield-admins``. This name will be used as our ``backend_roles`` in ``roles_mapping.yml``.
...
#. Create a new app. Configure the SAML settings while you create the app.
   ...
   #. Assign a name to the application and click on **Next**. In our case, we assign the name ``fortishield-sso-app``.
   ...
   #. In the **Configure SAML** menu, you’ll find the **SAML Settings** section, modify the following parameters:
      ...
      - **Single sign on URL**: input ``https://<FORTISHIELD_DASHBOARD_URL>/_opendistro/_security/saml/acs`` and replace the ``<FORTISHIELD_DASHBOARD_URL>`` field with the corresponding URL.
      - **Audience URI (SP Entity ID)**: input ``fortishield-saml``. This is the ``SP Entity ID`` value which will be used later in the ``config.yml`` on the Fortishield indexer instance.
      - **Other Requestable SSO URLs**: click on **Show Advanced Settings** to access this option. Input ``https://<FORTISHIELD_DASHBOARD_URL>/_opendistro/_security/saml/acs/idpinitiated`` and replace the ``<FORTISHIELD_DASHBOARD_URL>`` field with the corresponding URL.
   ...
   #. In the **Group Attribute Statements** section put ``Roles`` as the name. The value for ``Roles`` will be used as the ``roles_key`` parameter in the Fortishield indexer configuration. For the filter field, select **Matches regex** and type ``.*``. 
...
#. Edit the ``/etc/fortishield-indexer/opensearch-security/roles_mapping.yml`` file and change the following values:
   ...
   Configure the ``roles_mapping.yml`` file to map the Okta group to the appropriate Fortishield indexer role. In our case, we map it to the  ``all_access`` role:
   ...
   Replace ``<GROUP_NAME>`` with the name you gave to your group in Step 3. In our case, this is ``fortishield-admins``.
...
#. Check the value of ``run_as`` in the ``/usr/share/fortishield-dashboard/data/fortishield/config/fortishield.yml`` configuration file. If ``run_as`` is set to ``false``, proceed to the next step.
   ...
   #. Click **Create Role mapping** and complete the empty fields with the following parameters:
      ...
      - **Value**: Assign the name you gave to your group in Step 3 of Okta configuration, in our case, this is ``fortishield-admins``. 

      .. thumbnail:: /images/single-sign-on/okta/Fortishield-role-mapping.png
...
#. Test the configuration. Go to your Fortishield dashboard URL and log in with your Okta account. 
