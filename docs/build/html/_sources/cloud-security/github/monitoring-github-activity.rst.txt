...
.. meta::
  :description: Discover the way that Fortishield provides to monitor your organization's GitHub activity. Check out this section of our documentation to learn more about it.
...
This Fortishield module allows you to collect all the logs from GitHub using its API:
...
For **Fortishield** to successfully connect to the **GitHub API**, an authentication process is required. To do this, we must provide the name of the organization on GitHub and personal access tokens.
...
Fortishield provides a series of rules to catch different events on GitHub, for this example we will take the rule id ``91393`` which detects a ``GitHub Team add member`` action.
...
If Fortishield successfully connects to GitHub, the events raised above will trigger these rules and cause an alert like this:
