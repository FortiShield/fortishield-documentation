...
.. meta::
  :description: AWS Trusted Advisor is a service that helps users reduce cost by optimizing their AWS environment. Learn how to configure and monitor it with Fortishield.
...
Create an Amazon Kinesis Data Firehose delivery stream to be able to store the Trusted Advisor logs into the desired S3 bucket so Fortishield can process them.
...
#. Access the Fortishield configuration in **Settings** using the Fortishield UI or by manually editing the ``/var/ossec/etc/ossec.conf`` file in the host:
...
#. Add the following :doc:`AWS S3 module </user-manual/reference/ossec-conf/wodle-s3>` configuration to the file, replacing ``fortishield-aws-wodle`` with the name of the S3 bucket:
...
#. Save the changes and restart Fortishield to apply the changes. The service can be manually restarted using the following command outside the Fortishield UI:
