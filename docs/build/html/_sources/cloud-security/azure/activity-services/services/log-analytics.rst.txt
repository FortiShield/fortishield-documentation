...
.. meta::
  :description: Azure Log Analytics is a service that monitors Azure infrastructures offering query capabilities. Learn how to use Log Analytics with Fortishield in this section.
...
#. In the **Overview** section, copy the ``Workspace Id`` value. The Fortishield configuration needs it to make requests to the API.
...
Fortishield requires valid credentials to pull logs from Log Analytics. Take a look at the :ref:`credentials <graph_and_log_analytics_credentials>` section to learn how to generate a client secret so you can access the App registration.
...
Through the following configuration, Fortishield is ready to search for any query accepted by Azure Log Analytics. This example configuration includes a representative ``tag`` and will be scheduled for every Monday at 02:00, using an offset of one day, which means only the log data from the last day will be parsed:
...
The following rules are already included in Fortishield by default. With them, it it possible to monitor the infrastructure activity and get the related alerts.
...
Once the Fortishield configuration is set and the ``azure-logs`` module is running using the previous configuration, the event will be processed. The results can be checked in the Fortishield dashboard:
