...
.. meta::
  :description: The Fortishield module for Docker identifyes security incidents across containers alerting in real time. Learn more about this in this PoC.

Monitoring Docker events
========================

Docker automates the deployment of different applications inside software containers. The Fortishield module for Docker identifies security incidents across containers and alerts in real-time. In this use case, you configure Fortishield to monitor Docker events on an Ubuntu endpoint hosting Docker containers.
...
Perform the following steps to install Docker on the Ubuntu endpoint and configure Fortishield to monitor Docker events.
...
#. Edit the Fortishield agent configuration file ``/var/ossec/etc/ossec.conf`` and add this block to enable the ``docker-listener`` module:
...
-  **Error log**:
   ...
   .. code-block:: none
      ...
      fortishield-modulesd:docker-listener: ERROR: /usr/bin/env: ‘python’: No such file or directory
   
   **Location**: Fortishield agent log - ``/var/ossec/logs/ossec.log``
